...
function createSupabaseMock() {
  ...
  const createSelectBuilder = (table: keyof DataStore) => {
    ...
    const builder = {
      ...
      order: (field: keyof DataStore[typeof table][number]) => {
        ...
        const ordered = [...filtered].sort((a, b) => String(a[field]).localeCompare(String(b[field])))
      ...
      maybeSingle: () => {
        const filtered = applyFilters(dataStore[table], filters)
        const data = (limitValue ? filtered.slice(0, limitValue) : filtered)[0] ?? null
        return Promise.resolve({ data, error: null })
      }
    }
  ...
  const deleteRows = <T extends keyof DataStore>(
    ...
    const removed = rows.filter((row) => filters.every((filter) => filter(row)))
    dataStore = {
      ...dataStore,
      [table]: remaining
    }
    ...
    from: (table: keyof DataStore) => {
      return {
        ...
        update: (payload: Record<string, unknown>) => {
          return {
            eq: (field: string, value: unknown) => {
              const updated = updateRows(table, payload, [(row) => row[field] === value])
        ...
        delete: () => {
          return {
            eq: (field: string, value: unknown) => {
              const removed = deleteRows(table, [(row) => row[field] === value])
              return Promise.resolve({ data: removed, error: null })
            },
            in: (field: string, values: unknown[]) => {
              const removed = deleteRows(table, [(row) => values.includes(row[field])])
              return Promise.resolve({ data: removed, error: null })
            }
          }
        }
...
const loginAs = async (user: ReturnType<typeof userEvent.setup>, name: string, pin: string) => {
  await user.click(await screen.findByRole('button', { name }))
  await user.type(screen.getByPlaceholderText('PIN'), pin)
  await user.click(screen.getByRole('button', { name: `登录 ${name}` }))
}

const selectChild = async (user: ReturnType<typeof userEvent.setup>, name: string) => {
  const childSection = screen.getByRole('heading', { name: '孩子列表' }).closest('section')
  expect(childSection).not.toBeNull()

  await user.click(
    await within(childSection as HTMLElement).findByRole('button', { name: new RegExp(name) })
  )
}

const selectAccount = async (user: ReturnType<typeof userEvent.setup>, name: string) => {
  const accountSection = screen.getByRole('heading', { name: '账户列表' }).closest('section')
  expect(accountSection).not.toBeNull()

  await user.click(within(accountSection as HTMLElement).getByRole('button', { name: new RegExp(name) }))
...
describe('Home Bank UI', () => {
  ...
  it('renders parent view with accounts and balances', async () => {
    ...
    expect(await screen.findByRole('heading', { name: '孩子列表' })).toBeInTheDocument()
  ...
  it('filters transfer targets by currency', async () => {
    ...
    expect(within(select).queryByText('小儿子 - 零花钱')).not.toBeInTheDocument()
  })
  ...
  it('deletes a child and related accounts', async () => {
    ...
    await user.click(within(childRow as HTMLElement).getByRole('button', { name: '删除' }))

    expect(await screen.findByText('已删除孩子及关联账户。')).toBeInTheDocument()
  ...
  it('updates child name from list', async () => {
    ...
    await user.click(within(childRow as HTMLElement).getByRole('button', { name: '编辑' }))

    const input = within(childRow as HTMLElement).getByDisplayValue('小女儿')
    await user.clear(input)
    await user.type(input, '小宝')
    await user.click(within(childRow as HTMLElement).getByRole('button', { name: '保存' }))
  ...
  it('updates account name from list', async () => {
    ...
    await user.click(within(accountSection as HTMLElement).getByRole('button', { name: '编辑' }))

    const inputs = within(accountSection as HTMLElement).getAllByDisplayValue('目标基金')
  ...
  it('renders child view as read-only', async () => {
    loadMockData({
      app_users: [{ id: 'child-1', name: '大女儿', role: 'child', pin: '1111' }],
      accounts: [
        {
          id: 'acc-1',
          name: '大女儿-日常',
    ...
    expect(await screen.findByRole('heading', { name: '大女儿' })).toBeInTheDocument()

    expect(screen.queryByText('新增/扣减')).not.toBeInTheDocument()
    expect(screen.queryByText('同币种转账')).not.toBeInTheDocument()
    expect(screen.queryByRole('button', { name: '月结息' })).not.toBeInTheDocument()

    await user.click(await screen.findByRole('button', { name: /大女儿-日常/ }))
